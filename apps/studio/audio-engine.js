...
(function () {
  ...
  // ---- AIFF / AIFC PCM decoder ---------------------------------------------
  // Fallback for browsers whose Web Audio decodeAudioData can't handle AIFF
  // (notably Chrome). Parses uncompressed AIFF and the common uncompressed
  // AIFC variants (NONE/twos = big-endian, sowt = little-endian, fl32/fl64 =
  // float). Returns an AudioBuffer, or null if it isn't AIFF we can decode.
  function readExtendedFloat80(dv, off) {
    // 80-bit IEEE 754 extended precision (big-endian): used for AIFF sampleRate.
  ...
  function decodeAiff(ctx, arrayBuffer) {
    ...
    while (p + 8 <= arrayBuffer.byteLength) {
      const id = tag(p);
      const size = dv.getUint32(p + 4); // big-endian
      const body = p + 8;
      if (id === "COMM") {
        const numChannels = dv.getInt16(body);
        const numSampleFrames = dv.getUint32(body + 2);
        const sampleSize = dv.getInt16(body + 6); // bits per sample
        ...
        const dataOffset = dv.getUint32(body); // bytes of padding before samples
        ssndOffset = body + 8 + dataOffset;
        ssndSize = size - 8 - dataOffset;
      }
      p = body + size + (size & 1); // chunks are padded to even byte boundaries
    }
    ...
    for (let c = 0; c < ch; c++) {
      ...
      for (let i = 0; i < numSampleFrames; i++) {
        ...
        let s;
        if (isFloat) {
          s = bytesPerSample === 8 ? dv.getFloat64(o, little) : dv.getFloat32(o, little);
        } else if (bytesPerSample === 1) {
          s = dv.getInt8(o) / 128;
        } else if (bytesPerSample === 2) {
          s = dv.getInt16(o, little) / 32768;
        } else if (bytesPerSample === 3) {
          // 24-bit signed, assemble big- or little-endian
          const b0 = dv.getUint8(o), b1 = dv.getUint8(o + 1), b2 = dv.getUint8(o + 2);
          let v = little ? (b2 << 16) | (b1 << 8) | b0 : (b0 << 16) | (b1 << 8) | b2;
          if (v & 0x800000) v -= 0x1000000;
          s = v / 8388608;
        } else {
          s = dv.getInt32(o, little) / 2147483648;
        }
        out[i] = s;
      }
    }
  ...
  // Header-only duration (seconds) from an AIFF/AIFC COMM chunk, without
  // decoding the samples. Lets the playlist show a duration for AIFF even
  // though Chrome's <audio> can't read it for metadata. Accepts a partial
  // ArrayBuffer (a leading slice of the file) as long as it contains COMM,
  // which by convention precedes the large SSND chunk. Returns null if not
  // determinable from the given bytes.
  ...
  // ---- PCM encoders (WAV + AIFF) -------------------------------------------
  // Pure helpers: take an array of Float32 channel buffers + sample rate and
  // return an ArrayBuffer of 16-bit PCM in the requested container. Used by the
  // session recorder so mixes export as standard .wav / .aiff (not WebM).
  ...
  function encodeWavPCM16(channels, sampleRate) {
    ...
    for (let i = 0; i < numFrames; i++) {
      for (let c = 0; c < numCh; c++) { dv.setInt16(o, floatTo16(channels[c][i]), true); o += 2; }
    }
    return buf;
  }

  // 80-bit IEEE 754 extended float (big-endian), exact for integer sample rates.
  function writeFloat80(dv, off, value) {
    if (value <= 0) { for (let i = 0; i < 10; i++) dv.setUint8(off + i, 0); return; }
    const exp = Math.floor(Math.log2(value));
    const mant = BigInt(value) << BigInt(63 - exp); // value*2^(63-exp), exact for ints
  ...
  function encodeAiffPCM16(channels, sampleRate) {
    ...
    const dv = new DataView(buf);
    const wstr = (o, s) => { for (let i = 0; i < s.length; i++) dv.setUint8(o + i, s.charCodeAt(i)); };
    wstr(0, "FORM"); dv.setUint32(4, formSize); wstr(8, "AIFF");
    let p = 12;
    wstr(p, "COMM"); dv.setUint32(p + 4, commSize);
    dv.setInt16(p + 8, numCh); dv.setUint32(p + 10, numFrames); dv.setInt16(p + 14, 16);
    writeFloat80(dv, p + 16, sampleRate);
    p += 8 + commSize;
    wstr(p, "SSND"); dv.setUint32(p + 4, ssndSize); dv.setUint32(p + 8, 0); dv.setUint32(p + 12, 0);
    let o = p + 16;
    for (let i = 0; i < numFrames; i++) {
      for (let c = 0; c < numCh; c++) { dv.setInt16(o, floatTo16(channels[c][i]), false); o += 2; } // big-endian
    }
    return buf;
  }
