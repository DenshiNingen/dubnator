...
// Controls exposed to MIDI mapping. Handlers are built in App (they call the
// stable state setters), so this list is just id/label/type for the UI + the
// registration loop. Kept to setter-only controls (no momentary triggers) so
// the registered handlers are closure-safe.
const MIDI_CONTROLS = [
  ...
  // expanded coverage (toward full MIDI mapping)
  ...
  // transport / playlist (momentary: act once per press)
  ...
  // every remaining knob/fader (round 3) — deck/input levels, EQ, FX trims
  ...
  // 10-band graphic EQ (Deck A) — every band fader
  FREQS_10.map((f, i) => ({ id: `geqA.${i}`, label: `GEQ A ${f >= 1000 ? f / 1000 + "k" : f}`, type: "range" })),
  // 4-band parametric (Deck A) — freq / Q / gain per band
  [0, 1, 2, 3].flatMap((i) => [
    { id: `paramA${i}.freq`, label: `Param ${i + 1} Freq`, type: "range" },
    { id: `paramA${i}.q`, label: `Param ${i + 1} Q`, type: "range" },
    { id: `paramA${i}.gain`, label: `Param ${i + 1} Gain`, type: "range" },
  ]),
);
...
// Drag + resize state for a floating window. Returns a fixed-position style and
// pointer-down handlers for the titlebar (drag) and corner (resize), clamped to
// the viewport. Shared by FloatingWindow and the playlist (which has its own
// titlebar with deck tabs).
function useFloatingBox(initial, minW = 300, minH = 200) {
  const [box, setBox] = useState(() => {
    const w = (initial && initial.w) || 440, h = (initial && initial.h) || 520;
  ...
  // Holds the teardown for the in-flight drag/resize so it can be cleaned up on
  // pointerup, pointercancel, OR component unmount (e.g. ESC closes the window
  // mid-drag) — otherwise the window listeners leak and setBox fires on an
  // unmounted component.
  const activeCleanup = useRef(null);
  const dragWith = (e, compute) => {
    e.preventDefault();
    if (activeCleanup.current) activeCleanup.current();
    const move = (ev) => setBox((b) => compute(b, ev));
    const end = () => {
  ...
  useEffect(() => () => { if (activeCleanup.current) activeCleanup.current(); }, []);
  const startDrag = (e) => {
    // Don't start a drag from interactive titlebar controls.
    if (e.target.closest("button, .dot, .pl-deck-tab, input, select")) return;
    const s = { mx: e.clientX, my: e.clientY, x: box.x, y: box.y };
    dragWith(e, (b, ev) => ({
      ...b,
      x: Math.max(0, Math.min(window.innerWidth - 60, s.x + (ev.clientX - s.mx))),
      y: Math.max(0, Math.min(window.innerHeight - 36, s.y + (ev.clientY - s.my))),
    }));
  };
  const startResize = (e) => {
    e.stopPropagation();
    const s = { mx: e.clientX, my: e.clientY, w: box.w, h: box.h };
    // Only enforce the min size; don't cap to (viewport − position) or a window
    // dragged near an edge would snap to min and refuse to grow.
    dragWith(e, (b, ev) => ({
      ...b,
      w: Math.max(minW, s.w + (ev.clientX - s.mx)),
      h: Math.max(minH, s.h + (ev.clientY - s.my)),
    }));
  };
  // Re-clamp into the viewport when the browser window shrinks, so an open
  // window (and its titlebar — the only drag handle) can't end up off-screen.
  useEffect(() => {
    const onResize = () => setBox((b) => ({
      ...b,
      x: Math.max(0, Math.min(window.innerWidth - 60, b.x)),
      y: Math.max(0, Math.min(window.innerHeight - 36, b.y)),
      w: Math.min(b.w, window.innerWidth),
      h: Math.min(b.h, window.innerHeight),
    }));
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, []);
  const style = { position: "fixed", left: box.x, top: box.y, width: box.w, height: box.h, zIndex: 1200 };
  return { box, style, startDrag, startResize };
}

// Draggable + resizable floating window. Replaces the blocking modal overlay so
// the rack stays interactive behind it. Portaled to <body> so it escapes the
// .chassis transform (a transformed ancestor makes position:fixed resolve
// against the chassis, not the viewport, breaking the pointer→position map).
function FloatingWindow({ title, onClose, initial, minW = 300, minH = 200, className = "", children }) {
  const fb = useFloatingBox(initial, minW, minH);
  const node = (
    <div className={`floating-window panel with-screws ${className}`} style={fb.style}>
...
// === Visual keyboard map ===
// Each key: { k: face label, top, bot, color, w (width units, default 1) }
// `top` = primary/unshifted action (yellow/white). `bot` = shifted/alt action (chip).
// `color` group: siren | reverb | echo | sample | mic | filter | kill | deck | playlist | screen | mute
const KEY_ROWS = [
  // Row 0 — number row + Esc-equivalent
  [
  ...
  [
    ...
    { k: "Alt", w: 1.0, color: "blank" },
    { k: "Cmd", w: 1.2, color: "blank" },
    { k: "Space", w: 6.0, color: "playlist", top: "Playbar List View" },
    { k: "Cmd", w: 1.2, color: "blank" },
    { k: "Alt", w: 1.0, color: "blank" },
    ...
    <div className="kb-wrap">
        ...
        {KEY_ROWS.map((row, ri) => (
          <div className="kb-row" key={ri}>
            {row.map((kk, ki) => (
              <div
                key={ki}
                className={`kb-key kb-${kk.color || "blank"}${kk.color === "blank" ? " is-blank" : ""}${kk.k === "Shift" ? " kb-key-shift" : ""}`}
                style={{ flex: kk.w || 1 }}>
                <div className="kb-face">{kk.k}</div>
                {kk.top && (
                  <div className={`kb-top ${kk.topMom ? "is-mom" : ""}`}>{kk.top}</div>
                )}
                {kk.bot && (
                  <div className="kb-bot">⇧ {kk.bot}</div>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="kb-legend-row">
        ...
        <div className="kb-legend">
          {COLOR_LEGEND.map((l) => (
            <span key={l.c} className="kb-legend-item">
              <span className={`kb-legend-swatch kb-${l.c}`}></span>
              {l.n}
            </span>
          ))}
      ...
      <div className="kb-foot mono">English keyboard shortcuts · inspired by the Dub FX Live reference</div>
    </div>
  );
}
